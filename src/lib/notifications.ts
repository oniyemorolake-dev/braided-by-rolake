...
export async function notifyOwner(booking: Booking): Promise<{ ok: boolean; error?: string }> {
  ...
  const message = [
    ...
    `MEDIA CONSENT: ${formatMediaConsentLabel(booking.mediaConsent, booking.mediaFace)}`,
    '',
  ...
  const payload = {
    ...
    media_consent: formatMediaConsentLabel(booking.mediaConsent, booking.mediaFace),
